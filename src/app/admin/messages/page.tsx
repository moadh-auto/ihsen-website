...
export default function MessagesPage() {
        ...
        {NAV.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => router.push(item.href)}
              style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 12px', borderRadius:10, cursor:'pointer', background:active?'rgba(175,142,74,0.22)':'transparent', border:active?'1px solid rgba(175,142,74,0.45)':'1px solid transparent', color:active?'#d4a95e':'rgba(255,255,255,0.55)', width:'100%', textAlign: isAdminAr ? 'right' : 'left', transition:'all .2s', fontFamily:font, fontSize:13, fontWeight:active?700:400, position:'relative' }}
              ...
              {sideOpen && <span style={{ whiteSpace:'nowrap', flex:1 }}>{isAdminAr ? item.ar : item.fr}</span>}
              {sideOpen && item.id === 'messages' && unreadCount > 0 && (
                <span style={{ background:'#EF4444', color:'#fff', fontSize:10, fontWeight:800, borderRadius:100, padding:'1px 6px', fontFamily:'Inter', minWidth:16, textAlign:'center' }}>
                  {unreadCount}
                </span>
              )}
            </button>
  ...
  return (
    <div style={{ minHeight:'100vh', background:C.bg, display:'flex', fontFamily:font, direction:dir }}>
      <style>{`
      ...
      {/* Mobile overlay */}
      {isMobile && sideOpen && (
        <div onClick={() => setSideOpen(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.5)', zIndex:299 }} />
      )}
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0 }}>
        ...
        <div style={{ background:'#fff', borderBottom:`1px solid ${C.border}`, padding:'16px 24px', display:'flex', alignItems:'center', gap:12, position:'sticky', top:0, zIndex:10 }}>
          {isMobile && (
            <button onClick={() => setSideOpen(true)} style={{ background:'none', border:'none', cursor:'pointer', color:C.muted, padding:4 }}>
